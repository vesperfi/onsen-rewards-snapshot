...
# 1. `NODE_URL` and `MNEMONIC` are set in .env file and github secrets on repo for validation. Use https instead of wss.
#     Set optional `REPO`, `REVIEWER`, `ASSIGNEE` in .env file
...
# `REPO` value can be set to fork for testing.
if [[ -z $REPO ]]; then
